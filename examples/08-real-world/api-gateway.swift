...
struct ApiGatewayExample {
    static func main() throws {
        ...
        try step1_initializeGateway()
        try step2_loadDynamicSchemas()  
        try step3_setupRoutingRules()
        try step4_demonstrateRequestProcessing()
        try step5_showcaseVersionHandling()
        try step6_performanceAnalysis()
    ...
    private static func step1_initializeGateway() throws {
    ...
    private static func step2_loadDynamicSchemas() throws {
        ...
        ExampleUtils.printTable(services.reduce(into: [String: String]()) { result, service in
    ...
    private static func step3_setupRoutingRules() throws {
    ...
    private static func step4_demonstrateRequestProcessing() throws {
        ...
        for (index, request) in testRequests.enumerated() {
            ...
            if response.statusCode == 200 {
                successCount += 1
                print("  ✅ Request \(index + 1): \(request.method) \(request.path) - Success (\(String(format: "%.1f", processingTime * 1000))ms)")
    ...
    private static func step5_showcaseVersionHandling() throws {
    ...
    private static func step6_performanceAnalysis() throws {
...
class ApiGateway {
    ...
    func processRequest(_ request: ApiRequest) throws -> ApiResponse {
        ...
        performanceMonitor.recordRequest(route.targetService, duration: processingTime, success: finalResponse.statusCode == 200)
...
enum RequestTransformation {
    ...
    func apply(to request: ApiRequest, using registry: SchemaRegistry) throws -> ApiRequest {
        ...
        case .applyFieldMask(let fields):
            ...
            modifiedRequest = ApiRequest(method: request.method, path: request.path, body: filteredBody, metadata: modifiedRequest.metadata)
