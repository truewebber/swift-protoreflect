...
class MessageTransformer {
  ...
  func transform(_ message: DynamicMessage, from: String, to: String) throws -> DynamicMessage {
    ...
    for (sourceField, targetField) in rule.mappings where try message.hasValue(forField: sourceField) {
      let value = try message.get(forField: sourceField)
      try setNestedValue(&result, path: targetField, value: value)
    }

    // Apply defaults
    for (field, defaultValue) in rule.defaults where !(try result.hasValue(forField: field)) {
...
struct TransformRule {
  ...
  init(
    ...
    defaults: [String: Any] = [:],
    computed: [String: (DynamicMessage) -> Any] = [:]
  ) {
