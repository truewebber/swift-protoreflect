...
# Основные примеры для начала
swift run HelloWorld         # Ваш первый пример
swift run FieldTypes         # Демонстрация типов полей
swift run TimestampDemo      # Well-known types
swift run FieldMaskDemo      # Маски полей
```
...
### Метод 1: Через Package.swift (Рекомендуемый)
```bash
# Из папки examples/
cd examples
...
### Метод 2: Прямой запуск скрипта (если нужна отладка)
```bash
...
find . -name "*.swift" -path "./*/basic-usage/*" -o -path "./*/well-known-types/*" | sort
...
2. Убедитесь что вы находитесь в папке `examples/`
