...
struct CustomExtensionsExample {
    ...
    private static func demonstrateQueryExtensions() throws {
            ...
            func orderBy(_ fieldName: String, ascending: Bool = true) -> MessageCollection {
                ...
                let sortedMessages = messages.sorted { first, second in
                    guard let firstValue = try? first.get(forField: fieldName),
                          let secondValue = try? second.get(forField: fieldName) else {
                        return false
                    }
        ...
        let seniorEngineers = collection
        ...
        let topByExperience = collection
        ...
        for (dept, employees) in byDepartment.sorted(by: { $0.key < $1.key }) {
            let avgSalary = employees.compactMap { try? $0.get(forField: "salary") as? Double }.reduce(0, +) / Double(employees.count)
        ...
        ExampleUtils.printDataTable([[
            ...
            "Result Count": "\(engineeringEmployees.count) | \(highSalaryEmployees.count) | \(seniorEngineers.count) | \(topByExperience.count) | \(byDepartment.count) groups | \(allNames.count) names",
    ...
    private static func demonstrateDSLExtensions() throws {
        ...
        func createMessage(name: String, package: String, @MessageBuilder fields: () -> [FieldDescriptor]) -> (FileDescriptor, MessageDescriptor) {
        ...
        func initializeMessage(_ message: inout DynamicMessage, @ArrayBuilder<(String, Any)> values: () -> [(String, Any)]) throws {
        ...
        ExampleUtils.printDataTable([[
            ...
            "Traditional API": "Verbose method calls | Manual validation | Scattered setup | Limited patterns | If/else blocks",
            "DSL Approach": "Declarative syntax | Compile-time checks | Centralized definition | Result builders | Embedded conditions",
