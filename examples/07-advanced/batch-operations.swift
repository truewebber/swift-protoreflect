...
struct BatchOperationsExample {
    ...
    private static func demonstrateBatchCreation() throws {
        ...
        let creationData = [
            ...
            ["Metric": "Creation Time", "Value": "\(String(format: "%.3f", creationTime.time * 1000))ms", "Performance": "Excellent"],
            ["Metric": "Throughput", "Value": "\(String(format: "%.0f", Double(batchSize)/creationTime.time)) msg/s", "Performance": "High"],
    ...
    private static func demonstrateBatchSerialization() throws {
        ...
        let serializationData = [
            ["Format": "Binary", "Total Size": "\(totalBinarySize) bytes", "Avg Size/Record": "\(totalBinarySize/recordCount) bytes", "Throughput": "\(String(format: "%.0f", Double(recordCount)/binaryTime.time)) rec/s"],
            ["Format": "JSON", "Total Size": "\(totalJsonSize) bytes", "Avg Size/Record": "\(totalJsonSize/recordCount) bytes", "Throughput": "\(String(format: "%.0f", Double(recordCount)/jsonTime.time)) rec/s"],
            ["Format": "Efficiency", "Total Size": "\(String(format: "%.1f", Double(totalBinarySize)/Double(totalJsonSize)*100))% of JSON", "Avg Size/Record": "Binary saves \(totalJsonSize-totalBinarySize) bytes", "Throughput": "Binary is \(String(format: "%.1fx", jsonTime.time/binaryTime.time)) faster"]
        ]
    ...
    private static func demonstrateBatchValidation() throws {
        ...
        let validationData = [
            ["Category": "Valid Products", "Count": "\(validCount)", "Percentage": "\(String(format: "%.1f", validationRate))%"],
            ["Category": "Invalid Products", "Count": "\(products.count - validCount)", "Percentage": "\(String(format: "%.1f", 100.0 - validationRate))%"],
    ...
    private static func demonstrateBatchTransformation() throws {
        ...
        let transformationData = [
            ...
            ["Metric": "Processing Time", "Value": "\(String(format: "%.3f", transformTime.time * 1000))ms", "Performance": "Fast"],
            ["Metric": "Throughput", "Value": "\(String(format: "%.0f", Double(processedRecords.count)/transformTime.time)) rec/s", "Performance": "High"],
    ...
    private static func demonstrateParallelProcessing() throws {
        ...
        let parallelData = [
            ["Processing Type": "Sequential", "Time": "\(String(format: "%.3f", sequentialTime.time * 1000))ms", "Throughput": "\(String(format: "%.0f", Double(taskCount)/sequentialTime.time)) tasks/s", "Efficiency": "Baseline"],
            ["Processing Type": "Parallel", "Time": "\(String(format: "%.3f", parallelTime.time * 1000))ms", "Throughput": "\(String(format: "%.0f", Double(taskCount)/parallelTime.time)) tasks/s", "Efficiency": "\(String(format: "%.1fx", sequentialTime.time/parallelTime.time)) faster"],
            ["Processing Type": "Speedup", "Time": "\(String(format: "%.3f", (sequentialTime.time - parallelTime.time) * 1000))ms saved", "Throughput": "+\(String(format: "%.0f", Double(taskCount)/parallelTime.time - Double(taskCount)/sequentialTime.time)) tasks/s", "Efficiency": "\(String(format: "%.1f", (sequentialTime.time/parallelTime.time - 1) * 100))% improvement"]
        ]
    ...
    private static func demonstrateMemoryOptimization() throws {
        ...
        let memoryData = [
            ...
            ["Metric": "Total Processing Time", "Value": "\(String(format: "%.3f", totalProcessingTime * 1000))ms", "Benefit": "Consistent performance"],
