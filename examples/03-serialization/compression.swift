...
struct CompressionExample {
    static func run() throws {
        ...
        try step1_compressionAlgorithmComparison()
        try step2_adaptiveCompressionStrategy()
        try step3_streamingCompression()
        try step4_dataPatternOptimization()
        try step5_compressionMonitoringAnalytics()
    ...
    private static func step1_compressionAlgorithmComparison() throws {
        ...
        for (dataSetName, testData) in testDataSets {
            ...
            for (algorithmName, algorithm) in algorithms {
                ...
                print("    \(algorithmName): \(ExampleUtils.formatDataSize(result.compressedSize)) (\(String(format: "%.1f%%", ratio))) in \(String(format: "%.2f", result.compressionTime * 1000))ms")
            }
    ...
    private static func step2_adaptiveCompressionStrategy() throws {
        ...
        for (datasetName, data) in datasets {
            ...
            print("      Decompression: \(String(format: "%.2f", decompressionTime * 1000))ms (\(isValid ? "✅ Valid" : "❌ Error"))")
            print("")
        }
    }
    
    private static func step3_streamingCompression() throws {
            ...
            for batchIndex in 0..<(recordCount / batchSize) {
                ...
                do {
                    ...
                    if batchIndex % 5 == 0 {
                        ...
                        print("    Batch \(batchIndex): compressed \(ExampleUtils.formatDataSize(stats.totalOriginalSize)) → \(ExampleUtils.formatDataSize(stats.totalCompressedSize))")
                    }
                } catch {
        ...
        print("    Compression ratio: \(String(format: "%.1f%%", Double(stats.totalCompressedSize) / Double(stats.totalOriginalSize) * 100))")
    ...
    private static func step4_dataPatternOptimization() throws {
        ...
        for (patternName, dataGenerator) in patterns {
            ...
            print("      Original: \(ExampleUtils.formatDataSize(testData.count)) → \(ExampleUtils.formatDataSize(originalCompressed.count)) (\(String(format: "%.1f%%", originalRatio)))")
            print("      Optimized: \(ExampleUtils.formatDataSize(optimizedData.count)) → \(ExampleUtils.formatDataSize(optimizedCompressed.count)) (\(String(format: "%.1f%%", optimizedRatio)))")
            ...
            print("      Performance: \(String(format: "%.2f", originalTime * 1000))ms → \(String(format: "%.2f", optimizedTime * 1000))ms")
            print("")
        }
    }
    
    private static func step5_compressionMonitoringAnalytics() throws {
    ...
    private static func generateRandomTags(_ seed: Int) -> [String] {
        let allTags = ["compression", "data", "streaming", "optimization", "performance", "efficiency", "algorithm", "processing"]
...
class CompressionMonitor {
    ...
    func generateAnalyticsReport() -> AnalyticsReport {
        ...
        return AnalyticsReport(
            ...
            totalDataProcessed: 1000000, // Simplified
...
func createTextHeavyData() -> Data {
    let words = ["compression", "algorithm", "optimization", "performance", "efficiency", "data", "processing", "streaming"]
    var text = ""
