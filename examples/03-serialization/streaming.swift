...
struct StreamingExample {
    ...
    let (_, streamingTime) = try ExampleUtils.measureTime {
      for batchIndex in 0..<(datasetSize / batchSize) {
        ...
        for record in batchRecords {
          ...
          _ = lengthPrefix.withUnsafeBytes {
            outputStream.write($0.bindMemory(to: UInt8.self).baseAddress!, maxLength: lengthPrefix.count)
          }
          _ = recordData.withUnsafeBytes {
            outputStream.write($0.bindMemory(to: UInt8.self).baseAddress!, maxLength: recordData.count)
          }
        ...
        if batchIndex % 10 == 0 {
          print(
            "    Batch \(batchIndex + 1)/\(datasetSize / batchSize): \(ExampleUtils.formatDataSize(totalSerialized)) written"
          )
        }
      }
    }
  ...
  private static func step5MemoryOptimizationTechniques() throws {
    ...
    print(
      "    Pool efficiency: \(String(format: "%.1f%%", Double(messagePool.poolHits) / Double(messagePool.poolHits + messagePool.poolMisses) * 100))"
    )
    ...
    print(
      "    Memory efficiency: \(String(format: "%.1f%%", Double(lazyDataset.loadedCount) / Double(lazyDataset.totalSize) * 100))"
    )
  ...
  private static func processBatch(_ batch: [DynamicMessage], batchNumber: Int) throws -> Double {
    ...
    if batchNumber % 10 == 0 {
      print(
        "    Processed batch \(batchNumber): \(batch.count) records, avg score: \(String(format: "%.2f", batchScoreSum / Double(batch.count)))"
      )
    }
