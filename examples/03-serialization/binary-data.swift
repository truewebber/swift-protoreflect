...
struct BinaryDataExample {
    ...
    private static func step1_bytesFieldsHandling() throws {
        ...
        print("    Overhead: \(String(format: "%.1f%%", calculateOverhead(original: textData.count + imageHeader.count + cryptoKey.count + configData.count, serialized: binaryData.count)))")
    ...
    private static func step2_dataEncodingFormats() throws {
        ...
        let encodingFormats = [
            ...
            ("Binary", originalData.map { String($0, radix: 2).padding(toLength: 8, withPad: "0", startingAt: 0) }.joined(separator: " "))
        ]
        ...
        for (format, encoded) in encodingFormats {
            ...
            print("      Size: \(encoded.count) chars (\(String(format: "%.1fx", Double(encoded.count) / Double(originalData.count))) expansion)")
        }
    ...
    private static func step5_dataCompressionTechniques() throws {
        ...
        for (algorithm, compressedData) in compressionTests {
            ...
            print("    \(algorithm): \(ExampleUtils.formatDataSize(compressedData.count)) (\(String(format: "%.1f%%", savings)) saved)")
    ...
    // Protocol functions
    private static func createCustomProtocolHeader(version: UInt8, messageType: UInt8, flags: UInt8, sequenceNumber: UInt32) -> Data {
        var header = Data()
    ...
    private static func parseCustomProtocolHeader(_ header: Data) -> (version: UInt8, messageType: UInt8, flags: UInt8, sequenceNumber: UInt32) {
        ...
        let sequenceNumber = (UInt32(header[4]) << 24) |
                           (UInt32(header[5]) << 16) |
                           (UInt32(header[6]) << 8) |
                           UInt32(header[7])
