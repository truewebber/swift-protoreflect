...
struct ServiceDiscoveryExample {
    static func main() throws {
        try ExampleUtils.printHeader("gRPC Service Discovery")
    ...
    private static func demonstrateServiceCapabilities() throws {
        ...
        for serviceDescriptor in services {
            ...
            print("    \(serviceName) | \(crudSymbol)   | \(authSymbol)  | \(paginationSymbol)   | \(streamingSymbol)   | \(batchSymbol)")
        ...
        print("    CRUD Support: \(crudSupport)/\(services.count) services (\(Int(Double(crudSupport)/Double(services.count)*100))%)")
        print("    Auth Support: \(authSupport)/\(services.count) services (\(Int(Double(authSupport)/Double(services.count)*100))%)")
        print("    Pagination Support: \(paginationSupport)/\(services.count) services (\(Int(Double(paginationSupport)/Double(services.count)*100))%)")
    }
    ...
    private static func demonstrateHealthChecking() throws {
        ...
        print("\n  📊 Aggregate Health Metrics:")
        print("    Healthy Services: \(healthyServices)/\(services.count) (\(Int(Double(healthyServices)/Double(services.count)*100))%)")
    ...
    private static func createUserService() throws -> ServiceDescriptor {
        ...
        userService.addMethod(ServiceDescriptor.MethodDescriptor(
            name: "DeleteUser", inputType: "DeleteUserRequest", outputType: "Empty"
        ))
        userService.addMethod(ServiceDescriptor.MethodDescriptor(
    ...
    private static func createOrderService() throws -> ServiceDescriptor {
        ...
        orderService.addMethod(ServiceDescriptor.MethodDescriptor(
            name: "CreateOrder", inputType: "CreateOrderRequest", outputType: "Order"
        ))
        orderService.addMethod(ServiceDescriptor.MethodDescriptor(
            name: "GetOrder", inputType: "GetOrderRequest", outputType: "Order"
        ))
        orderService.addMethod(ServiceDescriptor.MethodDescriptor(
            name: "UpdateOrder", inputType: "UpdateOrderRequest", outputType: "Order"
        ))
        orderService.addMethod(ServiceDescriptor.MethodDescriptor(
    ...
    private static func createPaymentService() throws -> ServiceDescriptor {
        ...
        paymentService.addMethod(ServiceDescriptor.MethodDescriptor(
            name: "ProcessPayment", inputType: "ProcessPaymentRequest", outputType: "PaymentResult"
        ))
        paymentService.addMethod(ServiceDescriptor.MethodDescriptor(
            name: "RefundPayment", inputType: "RefundPaymentRequest", outputType: "RefundResult"
        ))
    ...
    private static func createNotificationService() throws -> ServiceDescriptor {
        ...
        notificationService.addMethod(ServiceDescriptor.MethodDescriptor(
            name: "SendNotification", inputType: "SendNotificationRequest", outputType: "NotificationResult"
        ))
...
class MethodExplorer {
    func exploreMethod(_ method: ServiceDescriptor.MethodDescriptor, in service: ServiceDescriptor) throws -> MethodExplorationResult {
...
class ServiceCatalogBuilder {
    func buildCatalog(from services: [ServiceDescriptor]) throws -> ServiceCatalog {
        ...
        for serviceDescriptor in services {
            let packageName = serviceDescriptor.fullName.components(separatedBy: ".").dropLast().joined(separator: ".").isEmpty ? "default" : serviceDescriptor.fullName.components(separatedBy: ".").dropLast().joined(separator: ".")
    ...
    func search(term: String) -> [SearchResult] {
        ...
        for (_, services) in servicesByPackage {
            for service in services {
                ...
                for endpoint in service.endpoints {
                    if endpoint.method.lowercased().contains(term.lowercased()) {
                        results.append(SearchResult(
                            serviceName: service.name,
                            methodName: endpoint.method,
                            relevance: 0.8
