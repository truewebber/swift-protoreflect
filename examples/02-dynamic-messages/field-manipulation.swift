...
struct FieldManipulationExample {
  static func main() throws {
    ...
    try step3UconditionalUpdates()
    try step4UfieldValidationConstraints()
  ...
  private static func applySmartDefaults(_ record: inout DynamicMessage, descriptor: MessageDescriptor) throws {
    for field in descriptor.fields.values where !(try record.hasValue(forField: field.name)) {
      switch field.type {
      case .string:
  ...
  private static func demonstrateRollback(_ document: DynamicMessage, factory: MessageFactory) throws {
    ...
    // Copy all fields
    for field in document.descriptor.fields.values where try document.hasValue(forField: field.name) {
  ...
  private static func demonstrateDynamicFieldDiscovery(_ config: DynamicMessage) throws {
    ...
    for field in config.descriptor.fields.values where try config.hasValue(forField: field.name) {
      switch field.type {
