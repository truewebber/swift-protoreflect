...
struct DependencyResolutionExample {
  static func run() throws {
    ...
    try step1UcomplexDependencyGraphAnalysis()
    try step2UcircularDependencyDetection()
    try step3UtopologicalSorting()
  ...
  private static func step1UcomplexDependencyGraphAnalysis() throws {
    ...
    for (fileName, dependencies) in dependencyGraph.edges where !dependencies.isEmpty {
...
class ConditionalDependencyResolver {
  func resolve(_ graph: ConditionalDependencyGraph, with config: [String: Bool]) -> ConditionalResolutionResult {
    ...
    for (_, conditions) in graph.conditionalDependencies {
      for condition in conditions where evaluateCondition(condition.condition, with: config) {
