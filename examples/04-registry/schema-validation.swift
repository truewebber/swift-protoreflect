...
class CompanyNamingConventionRule: ValidationRule {
    ...
    func validate(_ schema: FileDescriptor) -> [CustomViolation] {
        ...
        for message in schema.messages.values {
            if !message.name.hasPrefix("Company") && message.name.contains("company") {
    ...
    func validate(_ schema: FileDescriptor) -> [CustomViolation] {
        ...
        for message in schema.messages.values {
            for field in message.fields.values {
                if securityFieldNames.contains(field.name.lowercased()) {
                    violations.append(CustomViolation(
                        ruleName: name,
...
class CustomValidator {
    ...
    func generateReport(_ schemas: [(String, FileDescriptor)]) -> OverallReport {
        ...
        let overallScore = schemas.isEmpty ? 0.0 : 
            schemas.map { validate($0.1).complianceScore }.reduce(0, +) / Double(schemas.count)
