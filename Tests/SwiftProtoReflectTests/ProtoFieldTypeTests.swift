...
class ProtoFieldTypeTests: XCTestCase {
    ...
    func testEdgeCasesInTypeConversion() {
        ...
        // Test integer types
        let integerTypes: [ProtoFieldType] = [.int32, .int64, .uint32, .uint64, .sint32, .sint64,
        ...
        // Test non-integer types
        let nonIntegerTypes: [ProtoFieldType] = [.float, .double, .bool, .string, .bytes,
        ...
        // Test non-floating-point types
        let nonFloatingPointTypes: [ProtoFieldType] = [.int32, .int64, .uint32, .uint64, .sint32, .sint64,
        ...
        let nonStringBytesTypes: [ProtoFieldType] = [.int32, .int64, .uint32, .uint64, .sint32, .sint64,
    ...
    func testWireFormatCompatibility() {
        ...
        for type in lengthDelimitedTypes {
            XCTAssertTrue(ProtoFieldType.isWireTypeCompatible(type, 2), "\(type) should be compatible with Length-delimited wire type")
        }
        ...
        let allTypes: [ProtoFieldType] = [.int32, .int64, .uint32, .uint64, .sint32, .sint64, .bool, .enum(nil),
                                        .fixed64, .sfixed64, .double, .string, .bytes, .message(nil),
                                        .fixed32, .sfixed32, .float, .group, .unknown]
        
        for type in allTypes {
            // Test invalid wire type
            XCTAssertFalse(ProtoFieldType.isWireTypeCompatible(type, 3), "\(type) should not be compatible with invalid wire type 3")
            XCTAssertFalse(ProtoFieldType.isWireTypeCompatible(type, 4), "\(type) should not be compatible with invalid wire type 4")
            XCTAssertFalse(ProtoFieldType.isWireTypeCompatible(type, 6), "\(type) should not be compatible with invalid wire type 6")
            XCTAssertFalse(ProtoFieldType.isWireTypeCompatible(type, -1), "\(type) should not be compatible with negative wire type")
        }
    }
    ...
    func testTypeCompatibility() {
        ...
        for type1 in numericTypes {
            for type2 in numericTypes {
                XCTAssertTrue(ProtoFieldType.areTypesCompatible(type1, type2),
        ...
        let primitiveTypes: [ProtoFieldType] = [.int32, .int64, .uint32, .uint64, .sint32, .sint64,
                                              .fixed32, .sfixed32, .fixed64, .sfixed64, .float, .double,
                                              .bool, .string, .bytes]
        
        for primitiveType in primitiveTypes {
            XCTAssertFalse(ProtoFieldType.areTypesCompatible(.message(nil), primitiveType),
                          "Message type should not be compatible with \(primitiveType)")
            XCTAssertFalse(ProtoFieldType.areTypesCompatible(.enum(nil), primitiveType),
        ...
        for type in allTypes where type != .unknown {
            XCTAssertFalse(ProtoFieldType.areTypesCompatible(.unknown, type),
                          "Unknown type should not be compatible with \(type)")
            XCTAssertFalse(ProtoFieldType.areTypesCompatible(type, .unknown),
                          "\(type) should not be compatible with unknown type")
        }
        
        // Test group type compatibility
        for type in allTypes where type != .group {
            XCTAssertFalse(ProtoFieldType.areTypesCompatible(.group, type),
                          "Group type should not be compatible with \(type)")
            XCTAssertFalse(ProtoFieldType.areTypesCompatible(type, .group),
