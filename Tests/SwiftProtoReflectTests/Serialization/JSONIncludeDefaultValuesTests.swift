...
final class JSONIncludeDefaultValuesTests: XCTestCase {
  ...
  func test_includeDefaults_int32Field_emitsZero() async throws {
  ...
  func test_includeDefaults_stringField_emitsEmpty() async throws {
    let desc = makeDescriptor()
    let msg = DynamicMessage(descriptor: desc)
    let serializer = makeSerializer(includeDefaults: true)
    let json = try await jsonDict(serializer.serialize(msg))
  ...
  func test_includeDefaults_boolField_emitsFalse() async throws {
    let desc = makeDescriptor()
    let msg = DynamicMessage(descriptor: desc)
    let serializer = makeSerializer(includeDefaults: true)
    let json = try await jsonDict(serializer.serialize(msg))
  ...
  func test_includeDefaults_doubleField_emitsZero() async throws {
    let desc = makeDescriptor()
    let msg = DynamicMessage(descriptor: desc)
    let serializer = makeSerializer(includeDefaults: true)
    let json = try await jsonDict(serializer.serialize(msg))
  ...
  func test_includeDefaults_repeatedField_emitsEmptyArray() async throws {
    let desc = makeDescriptor()
    let msg = DynamicMessage(descriptor: desc)
    let serializer = makeSerializer(includeDefaults: true)
    let json = try await jsonDict(serializer.serialize(msg))
  ...
  func test_includeDefaults_messageField_omitted() async throws {
    var desc = MessageDescriptor(name: "Msg", fullName: "test.Msg")
    desc.addField(
  ...
  func test_includeDefaults_fieldSetToNonDefault_emitted() async throws {
    let desc = makeDescriptor()
  ...
  func test_includeDefaults_mixedSetAndUnset_allPresent() async throws {
    let desc = makeDescriptor()
    var msg = DynamicMessage(descriptor: desc)
    try msg.set(Int32(42), forField: 1)
  ...
  func test_includeDefaults_false_unsetFieldsOmitted() async throws {
    let desc = makeDescriptor()
    let msg = DynamicMessage(descriptor: desc)
  ...
  func test_includeDefaults_respectsUseOriginalFieldNames() async throws {
    let desc = makeDescriptor()
    let msg = DynamicMessage(descriptor: desc)
  ...
  func test_includeDefaults_proto3OptionalNotSet_stillOmitted() async throws {
    var desc = MessageDescriptor(name: "Msg", fullName: "test.Msg")
    desc.addField(
      FieldDescriptor(
        name: "opt_value",
  ...
  func test_includeDefaults_mapField_emitsEmptyObject() async throws {
    var desc = MessageDescriptor(name: "Msg", fullName: "test.Msg")
