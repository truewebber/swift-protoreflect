...
final class JSONDeserializerValueStructTests: XCTestCase {
  ...
  func test_deserialize_valueNull_fromJSONNull() async throws {
    let msg = try await deserializer.deserialize(json("null"), using: valueDesc)
  ...
  func test_deserialize_valueNumber_fromJSONNumber() async throws {
    let msg = try await deserializer.deserialize(json("3.14"), using: valueDesc)
  ...
  func test_deserialize_valueString_fromJSONString() async throws {
    let msg = try await deserializer.deserialize(json(#""hello""#), using: valueDesc)
  ...
  func test_deserialize_valueBoolTrue_fromJSONBool() async throws {
    let msg = try await deserializer.deserialize(json("true"), using: valueDesc)
  ...
  func test_deserialize_valueBoolFalse_fromJSONBool() async throws {
    let msg = try await deserializer.deserialize(json("false"), using: valueDesc)
    XCTAssertTrue((try? msg.hasValue(forField: 4)) == true, "bool_value (field 4) should be set")
    XCTAssertFalse((try? msg.hasValue(forField: 2)) == true, "number_value (field 2) must NOT be set")
    XCTAssertEqual(try msg.get(forField: 4) as? Bool, false)
  }

  func test_deserialize_valueStruct_fromJSONObject() async throws {
    let msg = try await deserializer.deserialize(json(#"{"key":"val"}"#), using: valueDesc)
  ...
  func test_deserialize_valueList_fromJSONArray() async throws {
    let msg = try await deserializer.deserialize(json("[1,2,3]"), using: valueDesc)
  ...
  func test_deserialize_struct_fromPlainObject() async throws {
    let msg = try await deserializer.deserialize(json(#"{"name":"Alice","score":42}"#), using: structDesc)
  ...
  func test_deserialize_emptyStruct_fromEmptyObject() async throws {
    let msg = try await deserializer.deserialize(json("{}"), using: structDesc)
    let structVal = try StructHandler.createSpecialized(from: msg) as! StructHandler.StructValue
  ...
  func test_deserialize_listValue_fromPlainArray() async throws {
    let msg = try await deserializer.deserialize(json(#"["a","b","c"]"#), using: listValueDesc)
  ...
  func test_deserialize_emptyListValue_fromEmptyArray() async throws {
    let msg = try await deserializer.deserialize(json("[]"), using: listValueDesc)
    let listVal = try ListValueHandler.createSpecialized(from: msg) as! [StructHandler.ValueValue]
  ...
  func test_deserialize_deepNesting_roundtrips() async throws {
    ...
    // Deserialize back
    let restored = try await deserializer.deserialize(data, using: structDesc)
