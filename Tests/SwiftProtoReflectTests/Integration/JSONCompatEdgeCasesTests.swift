...
final class JSONCompatEdgeCasesTests: XCTestCase {
  ...
  func test_edge_emptyBytes_bidirectional() async throws {
    var proto = Testcompat_ScalarMessage()
  ...
  func test_edge_int64MaxMin_quotedStrings_bidirectional() async throws {
    var proto = Testcompat_ScalarMessage()
  ...
  func test_edge_zeroInt64_quotedZero_bidirectional() async throws {
    var proto = Testcompat_ScalarMessage()
    proto.int64Field = 0
    ...
    try await CompatHelpers.assertProtocToUs(proto: protoOpt, descriptor: optDesc, registry: registry) { msg in
  ...
  func test_edge_nanInfinity_float_bidirectional() async throws {
    ...
    try await CompatHelpers.assertProtocToUs(proto: protoNaN, descriptor: desc, registry: registry) { msg in
    ...
    try await CompatHelpers.assertProtocToUs(proto: protoInf, descriptor: desc, registry: registry) { msg in
      let val = try XCTUnwrap(try msg.get(forField: 2) as? Float)
    ...
    try await CompatHelpers.assertProtocToUs(proto: protoNegInf, descriptor: desc, registry: registry) { msg in
      let val = try XCTUnwrap(try msg.get(forField: 2) as? Float)
      XCTAssertTrue(val.isInfinite && val < 0)
    }
  ...
  func test_edge_nan_infinity_inJSON() async throws {
    let desc = CompatDescriptors.scalarMessage()
  ...
  func test_edge_emptyMessage_emptyJSON_bidirectional() async throws {
  ...
  func test_edge_emptyMessage_deserialize_bidirectional() async throws {
    let proto = Testcompat_EmptyCustom()
    let desc = CompatDescriptors.emptyCustom()
    ...
    try await CompatHelpers.assertUsToProtoc(
      dynamic: dynamic,
      registry: registry,
      protoType: Testcompat_EmptyCustom.self
    ) {
      decoded in
  ...
  func test_edge_fieldOrderIndependent_directionA() async throws {
  ...
  func test_edge_maxUint64_quotedString_bidirectional() async throws {
    var proto = Testcompat_ScalarMessage()
  ...
  func test_edge_veryLongString_bidirectional() async throws {
