...
class ProtoFieldDescriptorTests: XCTestCase {
    ...
    func testInitializationWithDefaultValue() {
        ...
        // When
        let field = ProtoFieldDescriptor(name: "field", number: 1, type: .int32, isRepeated: false, isMap: false, defaultValue: defaultValue)
    ...
    func testInitializationWithMessageType() {
        ...
        // When
        let field = ProtoFieldDescriptor(name: "message_field", number: 1, type: .message, isRepeated: false, isMap: false, messageType: messageType)
    ...
    func testInvalidFieldDescriptorMissingMessageType() {
        // Given
        let invalidField = ProtoFieldDescriptor(name: "message_field", number: 1, type: .message, isRepeated: false, isMap: false)
        
        // Then
        XCTAssertFalse(invalidField.isValid())
        XCTAssertEqual(invalidField.validationError(), "Message type field 'message_field' requires a messageType descriptor")
    }
    ...
    func testAllFieldTypes() {
        ...
        let messageType = ProtoMessageDescriptor(fullName: "TestMessage", fields: [], enums: [], nestedMessages: [])
        let messageField = ProtoFieldDescriptor(name: "field", number: 1, type: .message, isRepeated: false, isMap: false, messageType: messageType)
