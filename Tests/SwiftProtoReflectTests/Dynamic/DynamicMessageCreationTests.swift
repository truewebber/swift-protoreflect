...
final class DynamicMessageCreationTests: XCTestCase {
    ...
    override func setUp() {
        ...
        // Сложные типы
        complexMessage.addField(FieldDescriptor(
            name: "nested_message", 
            number: 16, 
            type: .message, 
    ...
    func testAllScalarTypesProto3() throws {
        ...
        let scalarValues: [String: Any] = [
            ...
            "int64_field": Int64(-9223372036854775808),
        ...
        XCTAssertEqual(try message.get(forField: "int64_field") as? Int64, -9223372036854775808)
        XCTAssertEqual(try message.get(forField: "uint32_field") as? UInt32, 4294967295)
        XCTAssertEqual(try message.get(forField: "uint64_field") as? UInt64, 18446744073709551615)
        XCTAssertEqual(try message.get(forField: "sint32_field") as? Int32, -2147483648)
        XCTAssertEqual(try message.get(forField: "sint64_field") as? Int64, -9223372036854775808)
        XCTAssertEqual(try message.get(forField: "fixed32_field") as? UInt32, 123456789)
        XCTAssertEqual(try message.get(forField: "fixed64_field") as? UInt64, 987654321012345)
        XCTAssertEqual(try message.get(forField: "sfixed32_field") as? Int32, -123456789)
        XCTAssertEqual(try message.get(forField: "sfixed64_field") as? Int64, -987654321012345)
    ...
    func testComplexTypesCreation() throws {
        ...
        let nestedMsg = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(12345),
            "name": "Nested Object"
        ])
    ...
    func testRepeatedFieldsCreation() throws {
        ...
        let nestedMsg1 = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(1),
            "name": "First"
        ])
        let nestedMsg2 = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(2),
            "name": "Second"
        ])
    ...
    func testDeepNestedMessageCreation() throws {
        ...
        let level3 = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(3),
            "name": "Level 3"
        ])
        
        // Уровень 2 - также простое вложенное сообщение, содержащее ссылку на Level 3 в виде строки
        let level2 = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(2),
    ...
    func testComplexNestedStructureWithAllFieldTypes() throws {
        ...
        let mapNestedMsg1 = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(10),
            "name": "Map Nested 1"
        ])
        let mapNestedMsg2 = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(20),
            "name": "Map Nested 2"
        ])
        
        // Основное сообщение для вложения
        let mainNestedMsg = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(999),
            "name": "Main Nested"
        ])
    ...
    func testMessageCloning() throws {
        ...
        let originalNested = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(42),
            "name": "Original"
        ])
        
        let original = try messageFactory.createMessage(from: complexMessage, with: [
    ...
    func testMessageValidation() throws {
        ...
        let validNested = try messageFactory.createMessage(from: nestedMessage, with: [
            "id": Int64(1),
            "name": "Valid"
        ])
        
        let validMessage = try messageFactory.createMessage(from: complexMessage, with: [
