...
class ProtoValueConversionTests: XCTestCase {
  ...
  func testConvertToBytes() {
    ...
    // Valid conversions - protoc allows same type
    XCTAssertEqual(
      ProtoValue.bytesValue(Data([0x01, 0x02, 0x03])).convertTo(fieldDescriptor: bytesDescriptor)?.getBytes(),
      Data([0x01, 0x02, 0x03])
    )
  ...
  func testConvertToEnum() {
    ...
    // Valid conversions - protoc allows same type
    XCTAssertNotNil(
      ProtoValue.enumValue(name: "TEST_VALUE", number: 1, enumDescriptor: enumDescriptor)
