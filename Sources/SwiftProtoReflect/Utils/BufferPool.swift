...
public final class BufferPool {
  ...
  private func pruneExcessBuffers() {
    ...
    while !largeBuffers.isEmpty
      && (totalSize > BufferPool.maxPoolSize || largeBuffers.count > BufferPool.maxBuffers / 3)
    {
    ...
    // Then medium buffers
    while !mediumBuffers.isEmpty
      && (totalSize > BufferPool.maxPoolSize || mediumBuffers.count > BufferPool.maxBuffers / 3)
    {
    ...
    while !smallBuffers.isEmpty
      && (totalSize > BufferPool.maxPoolSize || smallBuffers.count > BufferPool.maxBuffers / 3)
    {
